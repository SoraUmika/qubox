...
### 2026-02-28 — Phase 1 Legacy Compatibility Cleanup

**Classification: Moderate**

Applied the Phase 1 cleanup actions from the legacy backward-compatibility audit:
archived one-shot migration scripts out of the core package, removed the legacy
parity harness, and removed deprecated compatibility aliases in active runtime APIs.

**Summary:**

1. **Archived one-shot migration scripts to top-level tools**
  - Moved `qubox_v2/migration/pulses_converter.py` → `tools/pulses_converter.py`.
  - Moved `qubox_v2/migration/strip_raw_artifacts.py` → `tools/strip_raw_artifacts.py`.
  - Updated moved scripts to use absolute `qubox_v2.*` imports.
  - Updated `qubox_v2/migration/__init__.py` docs to reference archived locations.

2. **Removed legacy parity harness from core package**
  - Deleted `qubox_v2/verification/legacy_parity.py`.

3. **Relocated device-layout migration helper to migration guides**
  - Moved `tools/migrate_device_to_samples.py` → `docs/migration_guides/migrate_device_to_samples.py`.

4. **Deprecated waveform alias signaling upgraded**
  - Replaced `print()` deprecation messages for `delta` alias usage with
    `warnings.warn(..., DeprecationWarning, stacklevel=2)` in `qubox_v2/tools/waveforms.py`.

5. **Removed deprecated spectroscopy parameter alias**
  - Removed `depletion_len` from
    `resonator_spectroscopy()` and `resonator_power_spectroscopy()` in
    `qubox_v2/programs/builders/spectroscopy.py`.

6. **Removed legacy readout kwargs compatibility mapping**
  - Removed legacy kwargs extraction/mapping in
    `ReadoutGEDiscrimination.run()` (`update_measureMacro`, `k`, `k_g`, `k_e`) from
    `qubox_v2/experiments/calibration/readout.py`.
...
- `qubox_v2/verification/legacy_parity.py` (removed)
- `tools/pulses_converter.py` (moved from package)
- `tools/strip_raw_artifacts.py` (moved from package)
- `docs/migration_guides/migrate_device_to_samples.py` (moved from `tools/`)
- `docs/CHANGELOG.md`

---

### 2026-02-28 — Phase 2–4 Legacy Removal Completion

**Classification: Major**

Completed the full legacy-removal plan after Phase 1: removed legacy
experiment/gate modules and program shims, enforced strict context-mode session
requirements, migrated internal imports to the unified program API module, and
updated notebook and API-reference documentation to avoid legacy implementations.

**Summary:**

1. **Removed legacy runtime modules and shims**
  - Deleted `qubox_v2/experiments/legacy_experiment.py`.
  - Deleted `qubox_v2/experiments/gates_legacy.py`.
  - Deleted `qubox_v2/programs/cQED_programs.py` shim.

2. **Introduced canonical program API surface**
  - Added `qubox_v2/programs/api.py` as the stable re-export layer over
    builder modules.
  - Updated package exports in `qubox_v2/programs/__init__.py` to re-export
    from `.api`.
  - Migrated experiment and hardware import sites from legacy shim imports to
    `from ...programs import api as cQED_programs`.

3. **Tightened context-mode session behavior**
  - Updated `SessionManager` context-mode runtime behavior to require explicit
    `sample_id` + `cooldown_id` usage without legacy fallback loading paths.

4. **Notebook and tooling de-legacyfication**
  - Updated `tools/build_context_notebook.py` to generate pulse-train
    calibration cells without `gates_legacy` dependencies.
  - Updated `notebooks/post_cavity_experiment_context.ipynb` to remove direct
    imports/instantiations of deleted legacy gate classes.

5. **API reference cleanup**
  - Updated `README.md` (governing API reference) to remove references and
    examples based on `qubox_v2.experiments.gates_legacy`.

**Files affected:**

- `qubox_v2/programs/api.py` (new)
- `qubox_v2/programs/__init__.py`
- `qubox_v2/programs/cQED_programs.py` (removed)
- `qubox_v2/experiments/legacy_experiment.py` (removed)
- `qubox_v2/experiments/gates_legacy.py` (removed)
- `qubox_v2/experiments/session.py`
- `qubox_v2/experiments/*` (import migration to `programs.api`)

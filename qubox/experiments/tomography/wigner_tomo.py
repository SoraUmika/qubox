...
class StorageWignerTomography(ExperimentBase):
    ...
    def _build_impl(
        ...
        parity_wait_clks: int | None = None,
        st_therm_clks: int | None = None,
        base_disp: str = "disp_n0",
    ) -> ProgramBuildResult:
        attr = self.attr
        ...
        st_therm = self.resolve_override_or_attr(
            value=st_therm_clks,
            attr_name="st_therm_clks",
            owner="StorageWignerTomography",
            cast=int,
        )
        if parity_wait_clks is None:
            # Default: ~1 µs / 4ns = 250 clock cycles (pi/chi approximation)
            parity_wait_clks = 250

        prog = cQED_programs.storage_wigner_tomography(
            gates,
            attr.st_el, attr.qb_el, attr.ro_el,
            base_disp,
            x_arr, p_arr, base_alpha,
            r90_pulse,
        ...
        return ProgramBuildResult(
            ...
            params={
                ...
                "st_therm_clks": st_therm,
                "parity_wait_clks": parity_wait_clks,
                "x_len": int(len(x_arr)),
                "p_len": int(len(p_arr)),
            },

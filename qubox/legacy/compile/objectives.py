...
from qubox.legacy.gates.contexts import ModelContext, NoiseConfig
from qubox.legacy.gates.cache import ModelCache
from qubox.legacy.gates.noise import QubitT1T2Noise
from qubox.legacy.gates.fidelity import avg_gate_fidelity_superop
...
def density_matrix_fidelity(
    ...
    if metric == "hilbert_schmidt":
        # F = Tr(Ï_resultâ€  Ï_target) / d
        # For density matrices: F = Tr(Ï_result Ï_target)
        d = rho_target.shape[0]
        F = np.real(np.trace(rho_result @ rho_target)) / d
        return float(np.clip(F, 0.0, 1.0))
    
    elif metric == "bures":
        # F = [Tr(âˆš(âˆšÏ_target Ï_result âˆšÏ_target))]Â²
...
def apply_gates_to_density_matrix(
    ...
    Each gate is applied as a quantum channel: Ï â†’ Î£_i K_i Ï K_iâ€ 
    ...
    for gate in gates:
        ...
        # Apply channel: Ï â†’ Î£_i K_i Ï K_iâ€ 
...
class ObjectiveConfig:
    ...
    mode:
      ...
      - 'density_matrix': compare Ï_impl to Ï_target (mixed states)
    ...
    depth_weight:
      ...
      - Fewer gates â†’ less error accumulation â†’ higher fidelity

...
class ProgramRunnerGUI(QtWidgets.QMainWindow):
    """Generic liveâ€‘update GUI that can run **any** callable repeatedly,
    autoâ€‘generate parameter controls from its signature, and plot one
    key of the returned dict/tuple in real time.

    If the callable returns a custom object with an ``extract`` method
    (like Quantumâ€‘Machines' QUA result handles), the GUI tries a few
    ...
    def __init__(self, program_dict: dict[str, Callable], timer_ms: int = 500):
        ...
        self._last_keys: list[str] = []  # for the plotâ€‘key combo
    ...
    def _build_ui(self):
        ...
        # â”€â”€ Top bar: program selector â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        # â”€â”€ Parameter form (autoâ€‘generated) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        # â”€â”€ Plot area â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        self.plot_widget = pg.PlotWidget()
        self.curve = self.plot_widget.plot(pen="y")
        layout.addWidget(self.plot_widget, stretch=1)

        # â”€â”€ Bottom bar: plot key + start/stop â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    def _program_changed(self, name: str):
        """Called when user picks a different program from the dropâ€‘down."""
        if not name:
            return
        self.current_program_name = name
        self.current_program = self.program_dict[name]
        self.timer.stop()  # ensure we don't call the old one midâ€‘change
        ...
        for p in sig.parameters.values():
            # Ignore *args, **kwargs â†’ you could handle them if you like
    ...
    def _current_params(self) -> dict[str, object]:
        ...
        for name, widget in self.param_widgets.items():
            if isinstance(widget, QtWidgets.QSpinBox):
                ...
                text = widget.text()
                # bestâ€‘effort numeric conversion
                try:
    ...
    def _acquire(self):
        ...
        if y.dtype.kind == "c":  # complex â†’ magnitude
            y = np.abs(y)
...
def _normalise_programs(programs: "dict[str, Callable] | Callable") -> dict[str, Callable]:
    ...
    raise TypeError("programs must be a callable or a mapping of strâ†’callable")

...
from qubox.experiments.result import FitResult
from qubox.experiments.time_domain.coherence import T2Echo, T2Ramsey
from qubox.experiments.time_domain.rabi import PowerRabi, TemporalRabi
from qubox.experiments.time_domain.relaxation import T1Relaxation
...
def test_temporal_rabi_analysis_fits_projected_signal(monkeypatch):
    ...
    monkeypatch.setattr(
        "qubox.experiments.time_domain.rabi.project_complex_to_line_real",
    ...
    monkeypatch.setattr("qubox.experiments.time_domain.rabi.fit_and_wrap", fake_fit)
...
def test_t1_analysis_fits_projected_signal_and_persists_it(monkeypatch):
    ...
    monkeypatch.setattr(
        "qubox.experiments.time_domain.relaxation.project_complex_to_line_real",
    ...
    monkeypatch.setattr("qubox.experiments.time_domain.relaxation.fit_and_wrap", fake_fit)
...
def test_coherence_analyses_fit_projected_signal_and_persist_it(monkeypatch):
    ...
    monkeypatch.setattr(
        "qubox.experiments.time_domain.coherence.project_complex_to_line_real",
    ...
    monkeypatch.setattr("qubox.experiments.time_domain.coherence.fit_and_wrap", fake_fit)

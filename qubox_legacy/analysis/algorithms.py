...
def curve_intersections(x, y1, y2, *, assume_sorted=False):
    ...
    # Handle three cases:
    # 1) Regular crossing: denom != 0 → t = d0 / (d0 - d1)
    # 2) Flat overlap (d0 == d1 == 0): the curves coincide over the segment → return midpoint
    ...
    # y from either y1 or y2 interpolant (they’re equal at the crossing)
...
def optimal_threshold_empirical(Ig_rot, Ie_rot, weights=None):
    ...
    # To be safe, also consider below-min and above-max (though they’ll be terrible)
...
def samples_for_k_sigma_event(k, Y, two_sided=True):
    ...
    two_sided : bool, optional
        If True  → use |X - mu| >= k*sigma (both tails).
        If False → use X - mu >= k*sigma (upper tail only).

    Returns
    -------
    n : int
        Minimum number of samples needed so that P(≥ 1 success) ≥ Y.
    """
    if not (0 < Y < 1):
    ...
    # Need n such that: 1 - (1 - p)^n >= Y  →  (1 - p)^n <= 1 - Y
...
def compute_waveform_fft(
    ...
    if time_window is not None:
        ...
        if t_begin < 1:
            raise ValueError("t_begin must be >= 1 (since z[0] ↔ t=1).")
        if t_end < t_begin:
        ...
        t = sample_numbers * dt * 1e6  # μs
    else:
        z_windowed = z
        N = z.size
        sample_numbers = np.arange(1, N + 1)  # 1..N because z[0] ↔ t=1
        t = sample_numbers * dt * 1e6  # μs
    ...
    if domain_norm == "both":
        ...
        ax_freq.set_title(f"Zero-padded FFT (×{zero_pad_factor})" + window_note)
        ...
        ax.set_title(f"Zero-padded FFT (×{zero_pad_factor})" + window_note)
        if freq_range is not None:
...
# (You said you’ll place them somewhere and import them.)
# ==============================
...
def peak_score_robust(S: np.ndarray) -> float:
    """
    Cheap “is there a real peak?” score.

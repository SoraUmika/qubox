...
def power_rabi_model(g, A, g_pi, offset):
    ...
    oscillates as sin²(θ/2) where the rotation angle θ is (to first order) linear in the pulse gain.
    ...
    Parameters:
        ...
        g_pi   : float
                 Gain corresponding to a π pulse.
        offset : float
                 Baseline level.
...
def T2_ramsey_model(t, A, T2, n, f_det, phi, offset):
    ...
    In a Ramsey experiment the qubit is prepared with a π/2 pulse, left to evolve for time t,
    and then another π/2 pulse is applied. The resulting signal is a decaying cosine at the detuning frequency.
...
def chi_ramsey_model(t, P0, A, T2_eff, nbar, chi, t0):
    ...
    Model (all inputs in *seconds* and *rad s⁻¹*)::

        P_e(t) = P0 + A · exp(-t / T2_eff)
                       · exp[ -2 n̄ · sin²(χ (t + t0) / 2) ]
...
def kerr_ramsey_model(t,            # time array   [s]
    ...
    Model:
        f(t) = offset + A * exp(-t/T2) *
               | ⟨α| exp[ i ω_c t a†a + i K t/2 (a†)^2 a^2 ] |α⟩ |^2
    ...
    K        : float
        Self-Kerr strength K [rad/s per photon²].
    alpha    : complex
        Initial coherent-state amplitude α.
    T2       : float
    ...
    offset   : float
        Baseline level.
    n_max    : int, optional
        Fock-space cutoff.  If None, picks a value ≈ |α|² + 5√|α|.
    ...
    # Poisson weights  P_n = e^{-|α|²} |α|^{2n} / n!
    Pn = np.exp(-n_mean) * (n_mean**n) / factorial(n)

    # Phase for each n:  φ_n(t) = t·[ ω_c n + (K/2) n(n-1) ]
...
def kerr_ramsey_model_(t, Δ, K, α, T2=1e9, A=1.0, offset=0.0, n_max=None):
    """
    Overlap |<α|exp(i Δ t n + i K t n(n-1)/2)|α>|^2   (Δ = small detuning).
    """
    t = np.atleast_1d(t)
    n̄ = abs(α)**2
    if n_max is None:
        n_max = int(np.ceil(n̄ + 5*np.sqrt(max(n̄,1e-3))))
    n = np.arange(n_max+1)
    Pn = np.exp(-n̄)*(n̄**n)/factorial(n)

    φ = Δ*n + 0.5*K*n*(n-1)                   # rad/s
    Et = np.sum(Pn*np.exp(1j*np.outer(t, φ)), axis=1)
...
def poisson_with_offset_model(n_arr, lam, scale, offset):
    ...
    Parameters:
        n_arr  : array-like
                 Fock state number(s) (0, 1, 2, ...).
        lam    : float
                 Mean photon number λ (lambda), typically |α|² for coherent states.
        scale  : float
...
def qubit_pulse_train_model(
    ...
    *,
    ...
    delta : float
        Detuning accumulated over ONE pulse (Δ * τ), in radians.
    ...
    # Build unitary U = exp[-i (v · sigma)/2]

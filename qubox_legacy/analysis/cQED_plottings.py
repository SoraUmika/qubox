...
def display_fock_populations(
    ...
    Poisson distribution with scale and offset:
        ...
        p_n ≈ offset + scale * exp(-λ) λ^n / n!,   λ = |alpha|^2
    ...
    if fit_alpha:
        ...
        # Initial guess for λ from weighted mean of n
        if np.allclose(y, 0):
            lam0 = 0.5
        else:
            w = np.abs(y)
        ...
        pois_fit_res = generalized_fit(
            ...
            global_opt=True,
            plotting=False,
            eq_str="offset + scale * exp(-λ) λ^n / n!",
        )
...
def plot_wigner(W: np.ndarray,
    ...
    """
    Plot a Wigner function W(α) on the (Re α, Im α) plane.
...
def plot_IQ(datasets, labels=None, ax=None, **scatter_kwargs):
    ...
    datasets : array_like of complex or list/tuple thereof
        Your IQ shots as complex numbers (I + 1j·Q). You can pass a single
    ...
    for idx, (S, lbl) in enumerate(zip(datasets, labels)):
        I = np.real(S)
        Q = np.imag(S)
        # build per‐dataset kwargs: if value is list/tuple, take element[idx]
        this_kwargs = {}

...
def butterfly_metrics(
    ...
    """
    Compute F, Q, V, Λ_M, and the post-measurement transition matrix T
    ...
    det_L_threshold : float
        Minimum allowed |det(Λ_M)| before we consider the matrix invalid.
    ...
    # ---- Confusion matrix Λ_M: P(m1 | state_i) -------------------------
    ...
    if not Lambda_M_valid:
        note = (
            "Invalid Λ_M (low visibility / near-singular): "
            f"V={V:.4g}, det(Λ_M)={det_L:.3e} < {det_L_threshold:.3e}. "
        ...
        # --- Joint probabilities P(m2, m1 | ψ_i) ------------------------

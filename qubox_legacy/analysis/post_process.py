...
def proc_default_legacy(output: Output, axis: int = 0, targets=None, **kwargs) -> Output:
    """
    ...
    **kwargs : dict
        Ignored additional arguments (e.g. from program_manager broadcasting).
        Dict-like with `.extract()` and item access.
    demod_len : int
        Demodulation length for `demod2volts`.
    axis : int
        Axis along which to unwrap phases and demod.
    targets : list[tuple] | None
        List of (I_key, Q_key) or (I_key, Q_key, tag).
        - If None, defaults to [('I','Q')].
        - `tag` (optional) names the outputs: e.g., tag='A' → keys 'S_A', 'S_volts_A', etc.

    Outputs added per target (using tag T; empty T means no suffix):
        S{_T}, S_volts{_T}, Phases{_T}, uPhases{_T}
    The corresponding I/Q keys are removed if processed successfully.
    """

    if targets is None:
        # Back-compat single pair
        targets = [("I", "Q")]

    # If legacy single I/Q missing, warn and bail early (kept from your original)
    if targets == [("I", "Q")] and ('I' not in output or 'Q' not in output):
        print("Warning: cannot demodulate without I/Q.")
        return output

    for t in targets:
        # Unpack (I_key, Q_key[, tag])
        if len(t) == 2:
            I_key, Q_key = t
        ...
        if I_key not in output or Q_key not in output:
            print(f"Warning: cannot demodulate pair ({I_key}, {Q_key}) — missing key(s).")
            continue
        ...
        try:
            ...
            # Remove originals for this pair
            try:
                del output[I_key]
                del output[Q_key]
            except Exception:
                # If Output guards deletion, it's fine to keep them.
                pass
...
def ro_state_correct_proc(
    ...
    **Layout assumption (NEW / STRICT):**
    ...
    If `to_sigmaz=True`:
        - We store corrected <σ_z> = P_e - P_g with the SAME SHAPE
    ...
    Λ = np.asarray(confusion, dtype=float)
    if Λ.shape[0] != Λ.shape[1]:
        raise ValueError("Confusion matrix must be square (n_outcomes, n_states).")
    if np.linalg.det(Λ) == 0:
    ...
    def _to_PgPe(a: np.ndarray):
        ...
        # Build (…, 2) where last axis is [P_g, P_e]
        Pe = a
        Pg = 1.0 - Pe
    ...
    def _correct(PgPe: np.ndarray) -> np.ndarray:
        """
        Apply Λ⁻¹ to PgPe along the last axis, with basic physicality fixes.
        ...
        # p_true_flat[i] = Λ_inv @ flat[i]
        p_true_flat = (Λ_inv @ flat.T).T  # still (M, 2)
    ...
    for t in targets:
        ...
        # Convert Pe → PgPe (...,2) and remember original shape
        ...
        if to_sigmaz:
            # σ_z = P_g - P_e  (standard: |g>=|0> -> +1, |e>=|1> -> -1)
...
def qubit_proc(output: Output,
    ...
    to each resulting complex stream S{_tag} → States{_tag}.
    """
    ...
    if targets is None:
        # Backward-compat: single pair ("I","Q") → S (no suffix)
        suffixes = [""]
    else:
        suffixes = []
        for t in targets:

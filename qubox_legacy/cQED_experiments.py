...
from .program_manager import QuaProgramManager, RunResult, ExecMode
from .pulse_manager import PulseOperationManager, PulseOp, MAX_AMPLITUDE
from .macros.measure_macro import measureMacro
from .logging_config import temporarily_set_levels
from qubox.device_manager import DeviceManager
...
def create_clks_array(t_begin, t_end, dt, time_per_clk: float = 4):
    ...
    if (t_begin != orig_t_begin) or (t_end != orig_t_end) or (dt != orig_dt):
        warnings.warn(
            ...
            f"t_begin {orig_t_begin}→{t_begin}, "
            f"t_end {orig_t_end}→{t_end}, "
            f"dt {orig_dt}→{dt}",
            RuntimeWarning,
        )
...
class cQED_Experiment:
        ...
        **kwargs
        ...
        if load_devices:
            ...
            try:
                if load_devices is True:
                    ...
                    _logger.info("load_devices=True → instantiating all configured devices.")
                    dm.instantiate_all()

                elif isinstance(load_devices, (list, tuple, set)):
                    names = list(load_devices)
                    _logger.info(
                        "load_devices list/tuple/set → instantiating selected device(s): %s",
                        ", ".join(names) or "(none)",
                    )
                    if names:
                        dm.instantiate(names)
                    else:
                        _logger.info("Empty load_devices list → no devices instantiated.")
        ...
        if per_element_modes:
            for el, m in per_element_modes.items():
                ...
                _logger.info("Per-element override: %s → %s", el, _as_mode(m).name)
    ...
    def T2_echo(self, delay_end: int, dt: int, delay_begin: int = 8, r180: str = "x180", r90: str = "x90", n_avg: int = 1000) -> RunResult:
        ...
        # half_wait_clks: units such that total delay 2τ = half_wait_clks * 8 ns
        ...
        runres = self.quaProgMngr.run_program(
            ...
            processors=[
                pp.proc_default,
                # attach total delay 2τ in ns for analysis:
    ...
    def randomized_benchmarking(
        ...
        *,
        ...
        # ✅ Full list of Clifford unitaries (24)
        CLIFF_U = [clifford_U_from_seq(seq) for seq in CLIFFORD_1Q_SEQS]

        # ✅ Unitary-to-Clifford lookup
    ...
    def qubit_pulse_train_legacy(self, N_values, K=2, rotation_pulse="x180", n_avg=1000, r90_pulse="x90") -> RunResult:
        """
        Qubit pulse train experiment to calibrate π-pulse amplitude.
        
        For each N in N_values:
            |g⟩ --(π/2)--> superposition --[KN × rotation pulse]--> final state --> measure P_e(N)
        ...
        r90_pulse : str
            Name of the π/2 pulse (default: "x90")
        rotation_pulse : str
    ...
    def qubit_pulse_train(
        ...
        reference_pulse : str, optional
            Name of reference pulse (e.g., π/2 pulse). Default: "x90"
        ...
        The sequence for each N value:
            If run_reference=True:
                ...
                2. play(rotation_pulse * amp(0)) × N times (zero amplitude)
                3. measure (reference)
                4. thermalize
            
            5. play(reference_pulse)
            6. play(rotation_pulse) × N times (actual amplitude)
            7. measure
            8. thermalize
    ...
    def qubit_state_tomography(
        ...
        *,
        ...
        x90_pulse : str, optional
            Pulse name that implements +π/2 about X (used to map σ_y → Z).
            Default "x90".
        yn90_pulse : str, optional
            Pulse name that implements -π/2 about Y (used to map σ_x → Z).
            Default "yn90".
        ...
        -------
        runres : RunResult
            Contains ``"sx"``, ``"sy"``, ``"sz"`` (corrected ⟨σ⟩ values).
            - Single prep  → scalars.
            - P preps      → arrays of shape ``(P,)``.
        """
    ...
    def readout_ge_integrated_trace(
        ...
        *,
        ...
        #  - pulse_len % d == 0  → slice length is integer
        #  - (pulse_len // d) % 4 == 0 → slice length is multiple of 4 ns
        valid_pairs = [
            (d, pulse_len // d // 4)
    ...
    def readout_weights_optimization(
        ...
        *,
        ...
        if make_plots:
            ...
            ax3.set_title(r"|e⟩ − |g⟩ (normalized)")
        ...
        *,
        ...
        if show_analysis:
            ...
            if confusion_matrix is not None:
                print("\nMeasurement confusion matrix Λ_M = P(m1 | state_i):")
            ...
            # --------- Optional η-calibration summary ----------
    ...
    def readout_amp_len_opt(
        ...
        • For each drive length L_body (ns): use a waveform of length L_body + ringdown_ns:
            - first L_body has constant amplitude `base_voltage`
            - trailing ringdown_ns has zero amplitude
        • For each (gain, L_body): run GE discrimination → rotated weights + threshold + (mu/sigma)
        • For each (gain, L_body): run butterfly → F, Q (QND) using those rotated weights

        Conventions:
        • min_len, max_len, dlen, ringdown_len are all in ns.
        min_len/max_len/dlen refer to the *drive-on* part only.
        • ringdown_len is a fixed extra “drive-off” duration appended after the drive.
        • min_g, max_g, dg define the gain sweep (dimensionless).
        • All lengths are snapped to integer clock cycles (t_clk_ns, default 4 ns) and
        the total length (drive + ringdown) is constrained so that the clock count is a multiple of 4.
        • PulseOp.length and integration-weight length are specified in ns.
        """
        ...
        try:
            with temporarily_set_levels([qubox_logger, qm_logger], logging.WARNING):
                ...
                def _make_aligned_range_ns_and_clks(
                    ...
                    • Inputs are in ns.
                    • Convert to clocks, round *up* to multiples of `mult_clks`.
                    • Returns (lens_ns, lens_clks) for the *drive-on* part.
                    """
                ...
                def _ensure_base_rect_weights(pulse: str, L_ns: int, *, prefix: str = ""):
                    ...
                    Internally:
                    • L_clks = round(L_ns / t_clk_ns)
                    • enforce L_clks % 4 == 0 (QM constraint)
                    """
                    L_ns = int(L_ns)
                    ...
                    if L_clks % 4 != 0:
                        raise ValueError(
                            f"Integration weight length {L_ns} ns → {L_clks} clocks, "
                ...
                if ringdown_len is None:
                    ...
                    ringdown_ns_raw = 1e9 / kappa_hz  # τ = 1/κ, in ns
                else:
                ...
                for L_body_ns, L_body_clks, L_tot_ns, L_tot_clks, ro_pulse, ro_op in zip(
                    ...
                    if int(L_body_clks) % 4 != 0:
                        raise ValueError(
                            f"Drive length {L_body_ns} ns → {L_body_clks} clocks, not a multiple of 4."
                        )
                    if int(L_tot_clks) % 4 != 0:
                        raise ValueError(
                            f"Total length {L_tot_ns} ns → {L_tot_clks} clocks, not a multiple of 4."
                        )
    ...
    def drag_calibration_YALE(self, amps, base_alpha: float, n_avg: int) -> RunResult:
        ...
        # Rotation by π/2 to generate Y pulses from X pulses
        ...
        *,
        ...
        def _compute_post_selected_data(out: Output, **_):
            ...
            - We compute:
                ...
                Pg_n_joint_corr ≈ Pg_n_joint_core / (eta_g * eta_e)
           ...
           # out["Pg_n_joint_corr"] = Pg_n_joint_corr
            ...
            # Backward compat: keep Pg_n_joint as the corrected one OR keep core — choose one.
    ...
    def fock_resolved_state_tomography(
        ...
        *,
        ...
        sel_r180="sel_x180",             # selective π (|g,n> <-> |e,n>)
        rxp90="x90",                   # global +π/2 about x (maps σ_y→σ_z)
        rym90="yn90",                   # global -π/2 about y (maps σ_x→σ_z)
    ...
    def SPA_flux_optimization(
        ...
        *,
        ...
        they’re interpreted as absolute RO frequencies (Hz) and converted to IFs.
        ...
        with temporarily_set_levels([qubox_logger, qm_logger], logging.WARNING):
            for dc in tqdm(
                ...
                unit="pt",        # or 'step', 'dc', etc. – optional
        ...
        # 3) stack over DC → matrices [n_freqs, n_dc]
        I_mat = np.stack([np.asarray(rr.output["I"]) for rr in seg_results], axis=1)
        Q_mat = np.stack([np.asarray(rr.output["Q"]) for rr in seg_results], axis=1)
        mag_matrix = np.abs(I_mat + 1j * Q_mat)
        out = Output()

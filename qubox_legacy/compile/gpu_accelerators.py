...
Usage:
    from qubox.compile.gpu_accelerators import enable_jax_acceleration
...
if _JAX_AVAILABLE:
    ...
    def unitary_avg_fidelity_jax(U_impl: jnp.ndarray, U_target: jnp.ndarray) -> float:
        ...
        Favg = (d*Fp + 1)/(d+1), where Fp = |Tr(U†V)|^2 / d^2
        ...
        # Efficient trace: Tr(A† @ B) = sum(conj(A) * B)
...
def enable_jax_acceleration(verbose: bool = True) -> bool:
    ...
    try:
        from qubox.compile import evaluators
        ...
        if verbose:
            ...
            if _JAX_GPU_AVAILABLE:
                print(f"🚀 GPU detected: {get_device_info()}")
        ...
        if verbose:
            print(f"⚠️  Could not import qubox.compile.evaluators: {e}")
        return False
...
def disable_jax_acceleration(verbose: bool = True) -> bool:
    ...
    if not _ACCELERATION_ENABLED:
        if verbose:
            print("ℹ️  JAX acceleration is not currently enabled")
        return False
    ...
    try:
        ...
        if verbose:
            print("✓ JAX acceleration disabled, restored NumPy evaluators")
...
def benchmark_gpu_speedup(n_max: int = 11, num_gates: int = 8, iterations: int = 100) -> Dict[str, float]:
    ...
    """
    if not _JAX_AVAILABLE:
        print("⚠️  JAX not available, cannot run benchmark")
        return {}

...
try:
    ...
    cp = None
    cp_expm = None
    print("ℹ CuPy not found - using CPU (NumPy) only")
...
def enable_gpu(enable: bool = True) -> bool:
    ...
    if enable and not _GPU_AVAILABLE:
        print("⚠ GPU requested but CuPy not available. Install with: pip install cupy-cuda11x")
        return False
    _USE_GPU = enable and _GPU_AVAILABLE
    if _USE_GPU:
        print(f"✓ GPU acceleration ENABLED (Device: {cp.cuda.Device().name.decode()})")
    else:
        print("ℹ GPU acceleration DISABLED (using CPU)")
    return _USE_GPU
...
if __name__ == "__main__":
    ...
    if is_gpu_available():
        ...
        benchmark_gpu_vs_cpu(n=24, iterations=1000)  # 2x12 qubit⊗cavity

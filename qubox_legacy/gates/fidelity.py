import numpy as np
from qubox.gates.liouville import unitary_to_superop  # adjust import path if needed
...
def entanglement_fidelity_from_superop(S: np.ndarray, d: int) -> float:
    ...
    convention based on |Ω> = sum_i |i,i>.
...
def avg_gate_fidelity_superop(S_impl: np.ndarray, U_target: np.ndarray) -> float:
    ...
    We compute E' = U_target^\dagger ∘ E, then:
      Favg = (d * Fe(E') + 1) / (d + 1)
    """

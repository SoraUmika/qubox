...
def compose_kraus(K_after: list[np.ndarray], K_before: list[np.ndarray]) -> list[np.ndarray]:
    """
    Kraus set for composition:  E_after ∘ E_before

    If:
      E_before(ρ) = Σ_i A_i ρ A_i†
      E_after (ρ) = Σ_j B_j ρ B_j†

    then:
      (E_after∘E_before)(ρ) = Σ_{j,i} (B_j A_i) ρ (B_j A_i)†
    """
...
def kraus_to_superop(kraus: list[np.ndarray]) -> np.ndarray:
    """
    Liouville superoperator for ρ -> Σ_k K_k ρ K_k†:
        S = Σ_k (K_k ⊗ K_k*)
    """
    if len(kraus) == 0:
...
def embed_kraus_on_total(
    ...
    """
    Embed Kraus ops acting on a subsystem into total space H_left ⊗ H_right.
...
def _single_qubit_rotation(theta: float, phi: float) -> np.ndarray:
    """
    Ideal single-qubit rotation about equatorial axis (cos φ, sin φ, 0):
        U(θ, φ) = exp[-i θ/2 (cos φ σ_x + sin φ σ_y)]
    """
    theta = float(theta)
    phi = float(phi)
...
class Gate(ABC):
    ...
    def get_kraus(
        ...
        **kwargs,
        ...
        # Build *noise* Kraus on qubit, embedded into (qubit ⊗ cavity)
...
class SQR(Gate):
    ...
    Reference template source:
      - If ref_I_x180_wf/ref_Q_x180_wf are provided: use them (X-selective π template).
      - Otherwise: look up ref_sel_x180_pulse from Gate.mgr (robustly, with '_pulse' toggling).

    Waveform:
      w_tot(t) = Σ_n  scale_n * w0(t) * exp[-i(phi_n + d_alpha_n)] * exp(+i( (ω_det[n]+d_omega_n) t ))
    ...
    def waveforms(self, *, from_chi: bool | None = None, d_omega_is_hz: bool = False):
        ...
        # detunings Δf_n (Hz) -> omega_det (rad/s)
        if from_chi is None:
...
class SNAP(Gate):
    ...
    (A) include_unselective=False  -> two *selective* π pulses back-to-back
        Implements the user's intended mapping:
          θ=π  -> X180 then  X180
          θ=0  -> X180 then -X180

    (B) include_unselective=True   -> one *selective* π pulse + one *unselective* π pulse
        We rewrite this branch so it implements the SAME θ-mapping as (A), by choosing the
        selective pulse axis φ_sel(θ) relative to the fixed unselective axis φ_uns.

        Derivation (equatorial axes):
          R(π, φ_uns) R(π, φ_sel) = -exp(i(φ_uns-φ_sel) σz)
        Acting on |g> gives phase  -exp(-i(φ_uns-φ_sel)).
        Choose φ_sel so that this phase equals exp(i θ) up to a global constant.

        Taking φ_sel = θ + φ_uns - π  gives:
          -exp(-i(φ_uns-φ_sel)) = -exp(-i(φ_uns-(θ+φ_uns-π))) = -exp(-i(π-θ)) = exp(iθ)
        (exactly; no leftover global aside from 2π periodicity).

        So for φ_uns = 0 (X), φ_sel = θ - π:
          θ=π -> φ_sel=0  ( +X )
          θ=0 -> φ_sel=π  ( -X )
    ...
    def __init__(
        ...
        *,
        ...
        self.marker = marker  # if None, we’ll pull marker from mgr pulse metadata
    ...
    def waveforms(self, *, from_chi: bool | None = None, d_omega_is_hz: bool = False):
        ...
        if use_two_selective:
            # Two selective π back-to-back (FIXED mapping):
            #   θ=π -> X then X
            #   θ=0 -> X then -X
            win = 2 * N
            T_sel = win * dt
            ...
            for n in range(L):
                ...
                # Pulse 2 axis: φ2 = π - θ  (so θ=π -> 0, θ=0 -> π)
            ...
            # One selective π then append unselective π,
            # rewritten to produce the SAME θ mapping as above.

            # Unselective axis angle φ_uns in the equatorial plane:
            #   X => 0
            #   Y => π/2  (synthesized by multiplying by exp(iπ/2))
            ...
            for n in range(L):
                ...
                #   phase(|g>) after [selective π, then unselective π] equals exp(i θ_n)
                #   φ_sel = θ + φ_uns - π  (plus calibration correction dalp)
                phi_sel = (theta_n + phi_uns - np.pi) + dalp

                w_sel += scale_n * (w0 * np.exp(1j * phi_sel)) * np.exp(1j * (omega_n * t))

            # Unselective π template (no per-n detuning modulation here)
...
class Idle(Gate):
    ...
    def __init__(self, wait_time: int, target: Optional[str] = None):
        ...
        # convert ns → QUA clocks (4 ns units), rounded down to multiple of 4
    ...
    # ideal unitary: identity in qubit ⊗ cavity space
    def ideal_unitary(self, n_max: int) -> np.ndarray:
        n_levels = int(n_max) + 1
        dim = 2 * n_levels
...
class Measure(Gate):
    ...
    axis ∈ {"x", "y", "z", "none"} chooses measurement basis.

    - "x", "y", "z": real measurements in the corresponding basis.
    - "none" or None: a *null* measurement gate — a placeholder that
...
class GateArray(Gate):
    ...
    Behaves like a Gate:
      • registered in _GATE_REGISTRY via Gate.__init_subclass__
      • supports ideal_unitary(n_max)
      • has _serialize_params / _deserialize_params
      • has play() and build() in the Gate style
    """
    gates: Iterable[Gate]
    op_prefix: str = "GA"

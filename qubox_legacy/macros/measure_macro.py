...
import logging
import json
from ..pulse_manager import PulseOp  # single canonical import
...
def _normalize_weight_sets(val):
    """
    Accepts:
      - None → DEFAULT_WEIGHT_SETS["base"]
      - str  → key into DEFAULT_WEIGHT_SETS
    ...
    # User provided a sequence – validate it's 2x2 strings
    try:
...
class measureMacro:
    ...
    def set_pulse_op(
        ...
        *,
        ...
        weights : None | str | 2x2 sequence[str], optional
            - None → use DEFAULT_WEIGHT_SETS["base"]
            - str  → name of a preset in DEFAULT_WEIGHT_SETS
            - 2x2  → explicit [[I_w, Q_w], [I_w2, Q_w2]]
    ...
    def _update_readout_discrimination(cls, out: dict):
        ...
        # widths (I’d cast to float for consistency)
    ...
    def _restore_from_snapshot(cls, s: dict):
        ...
        # Discrimination / quality params — keep backward-compat logic
    ...
    # ------------------------------------------------------------------ #
    #  Snapshot ⇄ JSON helpers (for current state + stack entries)
    # ------------------------------------------------------------------ #
    @classmethod

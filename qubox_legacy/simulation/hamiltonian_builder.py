...
# -----------------------------------------------------------------------------
# Term: one time-dependent piece A(t) * e^(−i ω t) * op
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# Build H'(t) in LAB basis corresponding to rotation by U=exp(−i F t)
# -----------------------------------------------------------------------------
...
def build_rotated_hamiltonian(
    ...
    rotate_static_offdiag: bool = False,  # handle [H_static, F] ≠ 0
) -> Union[qt.Qobj, list]:
    """
    Return a QuTiP time-dependent Hamiltonian (list format) in the LAB basis
    that implements the exact frame transform H'(t) = U†(t) H(t) U(t) − F.

    • If H_frame_lab is None: no rotation, i.e., H(t) = H_static_lab + Σ terms.
    • If rotate_static_offdiag=False and [H_static, F]=0 (e.g., diagonal case),
      then the static base is simply H_static_lab − H_frame_lab and time-independent.
    • If rotate_static_offdiag=True, we decompose H_static in the frame basis and
      add off-diagonal elements as oscillating terms (to avoid double-counting).
    • Driven terms are also transformed element-wise; RWA can drop fast pieces.
    """
    if args is None:
        args = {}
    ...
    # 1) Diagonalize the frame: F = V D V†

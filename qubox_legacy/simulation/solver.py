...
def _compile_hamiltonian(H, tlist, args=None):
    ...
    for term in H[1:]:
        ...
        # Numeric constant → fold into static term
        try:
            c = complex(coeff)
...
def solve_lindblad(H, tlist, rho0, *, c_ops=None, e_ops=None,
    ...
    Wrapper for qt.mesolve that:
      • accepts kets or density matrices for rho0,
      • uses a dict for `options` (avoids FutureWarning in newer QuTiP),
      • lets you choose a progress bar (many builds read it from options).
    """

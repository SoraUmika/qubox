...
def kaiser_pulse_waveforms(
    ...
    **kwargs,
    ...
    Optional features:
      ...
      - detuning: apply a complex carrier exp(i 2π detuning t) in Hz (same convention as your DRAG function)
    ...
    detuning : float
        Detuning in Hz for complex modulation (baseband envelope multiplied by exp(i 2π detuning t)).
    subtracted : bool
    ...
    if alpha != 0.0:
        ...
        # Use (2π*anharmonicity - 2π*detuning) in rad/s
...
def CLEAR_waveform(
    ...
    ----------
    t_duration : int
        Total pulse length (in samples / clks / ns – be consistent).
    ...
    if isinstance(t_kick, Sequence) and not isinstance(t_kick, (str, bytes)):
        ...
        # Single value → use same length for all four segments
...
def design_clear_kicks_from_rates(
    ...
    ----------
    kappa_rad_s : float
        Resonator linewidth κ in rad/s.
    chi_rad_s : float
        Dispersive shift χ in rad/s (positive, we use ±χ for g/e).
    A_steady : float
        Steady-state measurement amplitude in volts (your usual readout level).
    segment_dt_s : float
        Duration of a single kick segment in seconds (τ).
    ...
    # For each state j ∈ {g, e}:
    #   α_j^(2) = s_j g_j A1 + g_j A2 = α_ss_j
    M = np.array(
        [
            [s_g * g_g, g_g],
            [s_e * g_e, g_e],
    ...
    # Start both states at steady state (α_ss_j), end at α_j^(5) = 0
    # α_j^(5) = s_j^2 α_ss_j + s_j g_j A4 + g_j A5 = 0
    rhs_down = -np.array(
...
def build_CLEAR_waveform_from_physics(
    ...
    """
    Wrapper around CLEAR_waveform using κ, χ, and dt to choose kick amplitudes.
...
def gaussian_amp_for_same_rotation(
    ...
    *,
    ...
    Assumptions (standard in RWA for weak/moderate drives):
      - Rotation angle θ ∝ ∫ Ω(t) dt  (pulse "area")
      - Ω(t) is proportional to your Gaussian envelope amplitude
      ...
      - The Gaussian width scales with duration via: sigma = dur / (2*n_sigma)
        so that the edges are at ±n_sigma from the center.
    ...
    # sigma = dur / (2*n_sigma)  (center at dur/2, edges at ±n_sigma*sigma)
    ...
    # area = amp * sigma * sqrt(2π) * erf( (dur/2) / (sqrt(2)*sigma) )
    # Here (dur/2)/(sqrt(2)*sigma) = n_sigma/sqrt(2), same for both pulses,
    # so erf(...) cancels; leaving amp ∝ 1/sigma ∝ 1/dur.

...
def power_rabi_model(g, A, g_pi, offset):
    ...
    Parameters:
        ...
        g_pi   : float
                 Gain corresponding to a Ï€ pulse.
        offset : float
                 Baseline level.
...
def T2_ramsey_model(t, A, T2, n, f_det, phi, offset):
    ...
    In a Ramsey experiment the qubit is prepared with a Ï€/2 pulse, left to evolve for time t,
    and then another Ï€/2 pulse is applied. The resulting signal is a decaying cosine at the detuning frequency.
...
def chi_ramsey_model(t, P0, A, T2_eff, nbar, chi, t0):
    ...
    nbar     : float
        Mean photon number |Î±|Â².
    chi      : float
        Dispersive shift Ï‡ **in Hz**
    t0       : float
...
def kerr_ramsey_model(t,            # time array   [s]
    ...
    Model:
        f(t) = offset + A * exp(-t/T2) *
               | âŸ¨Î±| exp[ i Ï‰_c t aâ€ a + i K t/2 (aâ€ )^2 a^2 ] |Î±âŸ© |^2
    ...
    omega_c  : float
        Cavity resonance frequency Ï‰_c [rad/s].
    K        : float
        Self-Kerr strength K [rad/s per photonÂ²].
    alpha    : complex
        Initial coherent-state amplitude Î±.
    T2       : float
    ...
    offset   : float
        Baseline level.
    n_max    : int, optional
        Fock-space cutoff.  If None, picks a value â‰ˆ |Î±|Â² + 5âˆš|Î±|.
    ...
    # Poisson weights  P_n = e^{-|Î±|Â²} |Î±|^{2n} / n!
    Pn = np.exp(-n_mean) * (n_mean**n) / factorial(n)

    # Phase for each n:  Ï†_n(t) = tÂ·[ Ï‰_c n + (K/2) n(n-1) ]
...
def kerr_ramsey_model_(t, Î”, K, Î±, T2=1e9, A=1.0, offset=0.0, n_max=None):
    """
    Overlap |<Î±|exp(i Î” t n + i K t n(n-1)/2)|Î±>|^2   (Î” = small detuning).
    """
    t = np.atleast_1d(t)
    nÌ„ = abs(Î±)**2
    if n_max is None:
        n_max = int(np.ceil(nÌ„ + 5*np.sqrt(max(nÌ„,1e-3))))
    n = np.arange(n_max+1)
    Pn = np.exp(-nÌ„)*(nÌ„**n)/factorial(n)

    Ï† = Î”*n + 0.5*K*n*(n-1)                   # rad/s
    Et = np.sum(Pn*np.exp(1j*np.outer(t, Ï†)), axis=1)
...
def poisson_with_offset_model(n_arr, lam, scale, offset):
    ...
    Parameters:
        n_arr  : array-like
                 Fock state number(s) (0, 1, 2, ...).
        lam    : float
                 Mean photon number Î» (lambda), typically |Î±|Â² for coherent states.
        scale  : float
...
def qubit_pulse_train_model(
    ...
    *,
    ...
    delta : float
        Detuning accumulated over ONE pulse (Î” * Ï„), in radians.

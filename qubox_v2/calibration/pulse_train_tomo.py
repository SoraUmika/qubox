...
from ..analysis import cQED_models
from IPython.display import clear_output

# ======================================================================================
# NOTE:
# - This file assumes you are in a QUA-like environment where these exist:
#     play(pulse_name: str, element: str)
#     align()
# - Gate-under-test still uses your QubitRotation(...).play() (unchanged).
# - The ONLY change requested: state prep uses primitive pulses (x90,y90,xn90,yn90,x180,y180)
#   and we call align() after prep pulses and after the N repetitions.
# ======================================================================================
...
# ----------------------------
# UPDATED: primitive-based state-prep wrapper (with align)
# ----------------------------
def make_state_prep(prep_fn, arb_rot, N: int):
    """
    Factory for experiment.qubit_state_tomography callback.

    prep_fn: callable that plays primitive pulses (or None)
    arb_rot: object with .play() (your gate under test)
    N:       number of repetitions

    Uses QUA `for_()` loop so the compiled program size is O(1) regardless of N.
    """
    N = int(N)

    def state_prep():
        if prep_fn is not None:
            prep_fn()          # primitive pulses
            align()            # align after prep pulses
        if N > 0:
            n_var = declare(int)
            with for_(n_var, 0, n_var < N, n_var + 1):
                arb_rot.play()
            align()            # align after the N repetitions
...
def run_pulse_train_tomography(
    *,
    ...
    each bundling all K initial-state preps.
    Total calls = len(N_values) + 1 sanity.

    Returns:
        meas:       dict[label] -> (len(N_values), 3)
        prep_check: dict[label] -> (3,)   (only if sanity_check else None)

    prep_defs maps labels -> prep_fn (callable) or None.
    """
    ...
    if sanity_check:
        ...
        bloch_all = extract_bloch_from_tomo(rr)  # (K, 3)

        prep_check = {}
        for i, key in enumerate(keys):
            prep_check[key] = bloch_all[i]
            print(f"Prep sanity check (tomography after prep only, N=0):")
    ...
    # ------------------------------------------------------------------
    # Initialise per-key storage
    meas = {key: np.zeros((len(N_values), 3), float) for key in keys}

    for ni, N in enumerate(N_values):
        clear_output(wait=True)

        # Build K callables — one per prep, all sharing the same N
        preps_for_N = []
        for key in keys:
...
# ----------------------------
# Model (uses your existing cQED_models.qubit_pulse_train_model)
# ----------------------------
...
def fit_pulse_train_model(
    *,
    ...
    z_eq: float = +1.0,
    # residual choice
    residual_mode: str = "dir",     # "dir", "raw", "both"

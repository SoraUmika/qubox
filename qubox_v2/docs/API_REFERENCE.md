...
| `cqed_params.json` | `to_cqed_seed()` / `save_cqed_params()` | `set_roles()` + element frequencies |
...
```python
def set_roles(
    self, *, qubit: str, readout: str, storage: str | None = None
) -> HardwareDefinition
```

Assigns element names to experiment roles (`qubit`, `readout`, `storage`).
...
| Check | Description |
|-------|-------------|
| 1 | `qubit` and `readout` roles required |
| 2 | Role element names must exist |
| 3 | Readout element must use `add_readout()` with `rf_in` |
| 4 | No duplicate RF output ports |
| 5 | RF output ports in range 1–5 |
| 6 | IF frequencies within ±400 MHz |
| 7 | External LO references must match element `lo_source` |
| 8 | LO frequencies must be positive |
| 9 | Digital input ports in range 1–10 |
| 10 | Cross-reference: `set_external_lo(device=X)` warns if `X` not defined via `add_device()` (warning only) |
...
# ── Roles & ADC offsets ──
hw_def.set_roles(qubit="transmon", readout="resonator", storage="storage")

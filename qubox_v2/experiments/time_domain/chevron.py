...
class TimeRabiChevron(ExperimentBase):
    ...
    def run(
        ...
        attr = self.attr
        if_freqs = np.arange(-if_span, if_span + 1e-12, df, dtype=int)
        pulse_clks = create_clks_array(4, max_pulse_duration, dt, time_per_clk=4)

        self.set_standard_frequencies()

        prog = cQED_programs.time_rabi_chevron(
            attr.qb_el, pulse, pulse_clks, pulse_gain,
            if_freqs, attr.qb_therm_clks, n_avg,
        )
        result = self.run_program(
            ...
            processors=[
                ...
                pp.proc_attach("detunings", if_freqs),
            ],
        )
...
class PowerRabiChevron(ExperimentBase):
    ...
    def run(
        ...
        prog = cQED_programs.power_rabi_chevron(
            attr.qb_el, pulse, int(pulse_duration / 4), gains,
            if_freqs, attr.qb_therm_clks, n_avg,
        )
        result = self.run_program(
            prog, n_total=n_avg,

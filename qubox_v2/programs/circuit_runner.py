...
from .measurement import MeasureSpec, build_readout_snapshot_from_macro
...
class Gate:
    ...
    def resolved_name(self, *, index: int) -> str:
        ...
        family = str(self.params.get("family") or self.name).strip().replace(" ", "")
...
class QuantumCircuit:
    ...
    def with_stable_gate_names(self) -> "QuantumCircuit":
        ...
        return QuantumCircuit(name=self.name, gates=named, metadata=dict(self.metadata))
    ...
    def draw_logical(
        ...
        *,
        ...
        for gate in circuit.gates:
            targets = [gate.target] if isinstance(gate.target, str) else list(gate.target)
            for tgt in targets:
                if tgt not in wires:
                    wires.append(tgt)
        wires = sorted(wires)
        wire_y = {w: i for i, w in enumerate(wires)}

        n_steps = max(1, len(circuit.gates))
        if figsize is None:
            figsize = (max(8.0, 1.8 * n_steps), max(2.5, 1.2 * max(1, len(wires))))

        fig, ax = plt.subplots(figsize=figsize)

        for wire, y in wire_y.items():
            ax.hlines(y=y, xmin=0.5, xmax=n_steps + 0.5, color="black", linewidth=1.0)
            ax.text(0.35, y, wire, va="center", ha="right", fontsize=10)

        for i, gate in enumerate(circuit.gates, start=1):
            targets = [gate.target] if isinstance(gate.target, str) else list(gate.target)
            label_parts = []
            family = gate.params.get("family") or gate.name
            label_parts.append(str(family))
            if "angle" in gate.params:
                label_parts.append(f"θ={gate.params['angle']}")
            elif "alpha" in gate.params:
                label_parts.append(f"α={gate.params['alpha']}")
            elif "op" in gate.params:
                label_parts.append(str(gate.params["op"]))
            if include_instance_name and gate.instance_name:
                label_parts.append(gate.instance_name)
            label = "\n".join(label_parts)

            for target in targets:
                y = wire_y[target]
                rect = Rectangle((i - 0.35, y - 0.28), 0.7, 0.56, facecolor="#dbeafe", edgecolor="#1e40af", linewidth=1.2)
                ax.add_patch(rect)
                ax.text(i, y, label, ha="center", va="center", fontsize=8)

        ax.set_xlim(0.2, n_steps + 0.8)
        ax.set_ylim(-0.7, max(0.7, len(wires) - 0.3))
        ax.set_xticks(range(1, n_steps + 1))
        ax.set_xticklabels([str(i) for i in range(1, n_steps + 1)])
        ax.set_yticks([])
        ax.set_title(f"Logical Circuit: {circuit.name}")

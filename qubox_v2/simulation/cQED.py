...
# -----------------------------------------------------------------------------
# circuitQED: cavityâ€“transmon model and utilities
# -----------------------------------------------------------------------------
class circuitQED:
    ...
    def _init_system_operators(self):
        ...
        # ---- cavity observables (Nc==1 â†’ trivial but well-typed) ----
        if Nc == 1:
        ...
        if Nq >= 2:
            ...
            # projectors |g,n>, |e,n| â€” when Nc==1, only n=0 exists
            self.proj = {"g": {}, "e": {}}
            if Nc == 1:
    ...
    def show_drive(self, tlist, mode="env", args=None, ax=None, legend=True, figsize=(8,4)):
        ...
        for d in self.drives:
            ...
            if m == "env":
                ...
                raise ValueError("mode âˆˆ {'env','lab','iq'}")
        ...
        for d in self.drives:
            ...
            if enc == "cos":
                ...
                terms += [
                    ...
                    Term(op=op,       omega=-wcar, envelope=env, label=f"{d['name']} a-"),
                    Term(op=op.dag(), omega=+wcar, envelope=env, label=f"{d['name']} aâ€ +"),
                    Term(op=op.dag(), omega=-wcar, envelope=env, label=f"{d['name']} aâ€ -"),
                ]
                ...
                if op.isherm:
                    ...
                    # Ladder operator: A b e^{-iwt} + A* bâ€  e^{+iwt}
                    def env_lower(t, args, dd=d): return A(t, args, dd)       # op, omega = -w
                    def env_raise(t, args, dd=d):  return Aconj(t, args, dd)  # opâ€ , omega = +w
                    terms += [
    ...
    def initial_state_excited(self):
        """Cavity vacuum âŠ— qubit excited."""
        psi_cavity = qt.basis(self.Nc, 0)
